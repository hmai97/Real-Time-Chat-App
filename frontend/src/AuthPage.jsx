...
const AuthPage = (props) => {
  ...
  // Track whether the user should see the login or sign-up form
  ...
  const onLogin = (e) => {
    ...
    axios
      ...
      .then((r) => {
        props.onAuth({ ...r.data, secret }); // Successful login
      })
      .catch((e) => {
        if (e.response.status === 404) { // User not found, switch to signup
          setShowSignup(true);
        } else {
          console.log(JSON.stringify(e.response.data)); // Handle other errors
        }
      });
  };
  ...
  const onSignup = (e) => {
    ...
    axios
      ...
      .then((r) => {
        // After signing up, go back to login with the same credentials
